      ...
      <div className="app-body">
        <nav className="sidebar">
          ...
          {isAdmin && <><div className="sb-sep" /><div className="sb-label">Admin</div>
            ...
            <div className={`sb-item ${nav === "admin-nautiljon" ? "on" : ""}`} onClick={() => setNav("admin-nautiljon")}>➕ <span>Créer série</span></div>
          </>}
        </nav>
        <div className="main">
          {nav === "reading" && (() => {
          ...
          {nav === "admin-nautiljon" && isAdmin && <NautiljonCreator show={show} />}
        </div>
      </div>
...
// Admin — création manuelle d'une série/édition/volume directement dans la base
// Nautiljon locale (pour les séries absentes de nautiljon.com, ou ajoutées à la main).
// Web only, cf. décision utilisateur : pas d'équivalent côté appli Android.
const VIDE_VOLUME = () => ({ numero: "", titre: "", synopsis: "", imageFile: null });

function NautiljonCreator({ show }) {
  const [tab, setTab] = useState("nouvelle"); // "nouvelle" | "existante"

  // --- Nouvelle série ---
  const [sf, setSf] = useState({
    titre: "", synopsis: "", type: "", statut_vo: "", statut_vf: "", edition_nom: "Édition Standard",
    genres: "", themes: "", editeurs: "", auteurs: "", scenaristes: "", dessinateurs: "",
  });
  const [coverFile, setCoverFile] = useState(null);
  const [volumes, setVolumes] = useState([VIDE_VOLUME()]);
  const [creating, setCreating] = useState(false);
  const [createErr, setCreateErr] = useState("");

  const setVol = (i, patch) => setVolumes(vs => vs.map((v, idx) => idx === i ? { ...v, ...patch } : v));
  const addVolRow = () => setVolumes(vs => [...vs, VIDE_VOLUME()]);
  const delVolRow = (i) => setVolumes(vs => vs.filter((_, idx) => idx !== i));

  const creerSerie = async () => {
    if (!sf.titre.trim()) { setCreateErr("Le titre est obligatoire."); return; }
    setCreateErr(""); setCreating(true);
    try {
      const fd = new FormData();
      Object.entries(sf).forEach(([k, v]) => fd.set(k, v));
      if (coverFile) fd.set("image", coverFile);
      // Une entrée vol_image par ligne de volume, MÊME quand il n'y a pas d'image :
      // le backend associe vol_image[i] à vol_numero[i]/vol_titre[i] par position
      // (comme le ferait un vrai <input type="file"> non rempli dans un <form> HTML).
      volumes.forEach(v => {
        fd.append("vol_numero", v.numero || "");
        fd.append("vol_titre", v.titre || "");
        fd.append("vol_synopsis", v.synopsis || "");
        fd.append("vol_image", v.imageFile || new Blob([]), v.imageFile ? v.imageFile.name : "");
      });
      const r = await api.createNautiljonSerie(fd);
      show(`✅ Série créée (${r.nb_volumes} volume(s))`);
      setSf({ titre: "", synopsis: "", type: "", statut_vo: "", statut_vf: "", edition_nom: "Édition Standard", genres: "", themes: "", editeurs: "", auteurs: "", scenaristes: "", dessinateurs: "" });
      setCoverFile(null);
      setVolumes([VIDE_VOLUME()]);
    } catch (e) { setCreateErr(e.message); }
    setCreating(false);
  };

  // --- Ajouter à une série existante (édition / volume) ---
  const [search, setSearch] = useState("");
  const [results, setResults] = useState([]);
  const [searching, setSearching] = useState(false);
  const [serie, setSerie] = useState(null); // {details, editions}
  const [loadingSerie, setLoadingSerie] = useState(false);

  const runSearch = async () => {
    if (!search.trim()) return;
    setSearching(true);
    try { const r = await api.nautiljonSearch(search.trim(), 24, 0); setResults(r.results || []); }
    catch (e) { show(e.message); }
    setSearching(false);
  };

  const pickSerie = async (url) => {
    setLoadingSerie(true); setResults([]);
    try { setSerie(await api.nautiljonManga(url)); }
    catch (e) { show(e.message); }
    setLoadingSerie(false);
  };

  const [nef, setNef] = useState({ nom: "", statut: "" });
  const [nefErr, setNefErr] = useState("");
  const ajouterEdition = async () => {
    if (!nef.nom.trim()) { setNefErr("Le nom de l'édition est obligatoire."); return; }
    setNefErr("");
    try {
      await api.addNautiljonEdition(serie.details.url, nef.nom.trim(), nef.statut.trim());
      show("✅ Édition ajoutée");
      setNef({ nom: "", statut: "" });
      setSerie(await api.nautiljonManga(serie.details.url));
    } catch (e) { setNefErr(e.message); }
  };

  const [nvEditionId, setNvEditionId] = useState("");
  const [nvf, setNvf] = useState({ numero: "", titre: "", synopsis: "" });
  const [nvImage, setNvImage] = useState(null);
  const [nvErr, setNvErr] = useState("");
  const ajouterVolume = async () => {
    if (!nvEditionId) { setNvErr("Choisissez une édition."); return; }
    if (!nvf.numero.trim() && !nvf.titre.trim()) { setNvErr("Numéro ou titre requis."); return; }
    setNvErr("");
    try {
      const fd = new FormData();
      fd.set("edition_id", nvEditionId);
      fd.set("numero", nvf.numero.trim());
      fd.set("titre", nvf.titre.trim());
      fd.set("synopsis", nvf.synopsis.trim());
      if (nvImage) fd.set("image", nvImage);
      await api.addNautiljonVolume(fd);
      show("✅ Volume ajouté");
      setNvf({ numero: "", titre: "", synopsis: "" }); setNvImage(null);
      setSerie(await api.nautiljonManga(serie.details.url));
    } catch (e) { setNvErr(e.message); }
  };

  const editions = serie?.editions?.editions || [];

  return (
    <>
      <div className="sec-h"><span className="sec-t">Créer dans la base Nautiljon</span></div>
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        <button className={`btn ${tab === "nouvelle" ? "btn-p" : ""}`} onClick={() => setTab("nouvelle")}>➕ Nouvelle série</button>
        <button className={`btn ${tab === "existante" ? "btn-p" : ""}`} onClick={() => setTab("existante")}>📚 Série existante</button>
      </div>

      {tab === "nouvelle" && <div style={{ maxWidth: 560 }}>
        <div className="fld"><label>Titre *</label><input value={sf.titre} onChange={e => setSf(f => ({ ...f, titre: e.target.value }))} /></div>
        <div className="fld"><label>Synopsis</label><textarea rows={3} value={sf.synopsis} onChange={e => setSf(f => ({ ...f, synopsis: e.target.value }))} /></div>
        <div style={{ display: "flex", gap: 8 }}>
          <div className="fld" style={{ flex: 1 }}><label>Type</label><input value={sf.type} onChange={e => setSf(f => ({ ...f, type: e.target.value }))} placeholder="Manga, Josei…" /></div>
          <div className="fld" style={{ flex: 1 }}><label>Nom de l'édition</label><input value={sf.edition_nom} onChange={e => setSf(f => ({ ...f, edition_nom: e.target.value }))} /></div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <div className="fld" style={{ flex: 1 }}><label>Statut VO</label><input value={sf.statut_vo} onChange={e => setSf(f => ({ ...f, statut_vo: e.target.value }))} /></div>
          <div className="fld" style={{ flex: 1 }}><label>Statut VF</label><input value={sf.statut_vf} onChange={e => setSf(f => ({ ...f, statut_vf: e.target.value }))} /></div>
        </div>
        <div className="fld"><label>Genres</label><input value={sf.genres} onChange={e => setSf(f => ({ ...f, genres: e.target.value }))} placeholder="Action, Aventure" /></div>
        <div className="fld"><label>Thèmes</label><input value={sf.themes} onChange={e => setSf(f => ({ ...f, themes: e.target.value }))} placeholder="École, Combat" /></div>
        <div className="fld"><label>Éditeur(s) VF</label><input value={sf.editeurs} onChange={e => setSf(f => ({ ...f, editeurs: e.target.value }))} placeholder="Kana, Glénat" /></div>
        <div style={{ display: "flex", gap: 8 }}>
          <div className="fld" style={{ flex: 1 }}><label>Auteur(s)</label><input value={sf.auteurs} onChange={e => setSf(f => ({ ...f, auteurs: e.target.value }))} /></div>
          <div className="fld" style={{ flex: 1 }}><label>Scénariste(s)</label><input value={sf.scenaristes} onChange={e => setSf(f => ({ ...f, scenaristes: e.target.value }))} /></div>
          <div className="fld" style={{ flex: 1 }}><label>Dessinateur(s)</label><input value={sf.dessinateurs} onChange={e => setSf(f => ({ ...f, dessinateurs: e.target.value }))} /></div>
        </div>
        <div className="fld"><label>Cover</label><input type="file" accept="image/*" onChange={e => setCoverFile(e.target.files[0] || null)} /></div>

        <div style={{ marginTop: 16, padding: 14, background: "var(--c1)", borderRadius: "var(--r)", border: "1px solid var(--brd)" }}>
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10, color: "var(--t1)" }}>📖 Volumes</div>
          {volumes.map((v, i) => (
            <div key={i} style={{ display: "flex", gap: 6, alignItems: "flex-end", marginBottom: 8, flexWrap: "wrap" }}>
              <div className="fld" style={{ width: 90, margin: 0 }}><label>N°</label><input value={v.numero} onChange={e => setVol(i, { numero: e.target.value })} /></div>
              <div className="fld" style={{ flex: 1, minWidth: 140, margin: 0 }}><label>Titre</label><input value={v.titre} onChange={e => setVol(i, { titre: e.target.value })} /></div>
              <div className="fld" style={{ flex: 1, minWidth: 140, margin: 0 }}><label>Synopsis</label><input value={v.synopsis} onChange={e => setVol(i, { synopsis: e.target.value })} /></div>
              <div className="fld" style={{ margin: 0 }}><label>Image</label><input type="file" accept="image/*" onChange={e => setVol(i, { imageFile: e.target.files[0] || null })} /></div>
              <button className="btn btn-s btn-d" onClick={() => delVolRow(i)} disabled={volumes.length === 1} title="Retirer cette ligne">✕</button>
            </div>
          ))}
          <button className="btn btn-s" onClick={addVolRow}>+ Ajouter un volume</button>
        </div>

        <div className="card-err">{createErr}</div>
        <div style={{ marginTop: 12 }}><button className="btn btn-p" onClick={creerSerie} disabled={creating}>{creating ? "Création…" : "Créer la série"}</button></div>
      </div>}

      {tab === "existante" && <div style={{ maxWidth: 560 }}>
        <div className="fld">
          <label>Rechercher une série (base locale)</label>
          <div style={{ display: "flex", gap: 6 }}>
            <input value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === "Enter" && runSearch()} placeholder="Titre…" />
            <button className="btn btn-p" onClick={runSearch} disabled={searching}>{searching ? "…" : "🔍"}</button>
          </div>
        </div>
        {results.length > 0 && <div style={{ maxHeight: 240, overflowY: "auto", marginBottom: 14 }}>
          {results.map(r => (
            <div key={r.url} style={{ padding: "6px 8px", background: "var(--c1)", borderRadius: 4, marginBottom: 2, fontSize: 12, display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }} onClick={() => pickSerie(r.url)}>
              <span style={{ flex: 1, color: "var(--t2)" }}>{r.title}</span>
              <button className="btn btn-s">Sélectionner</button>
            </div>
          ))}
        </div>}

        {loadingSerie && <div className="loading"><div className="spinner" /></div>}

        {serie && !loadingSerie && <>
          <div style={{ padding: 10, background: "var(--c1)", borderRadius: "var(--r)", border: "1px solid var(--brd)", marginBottom: 14 }}>
            <div style={{ fontWeight: 600, color: "var(--t1)" }}>{serie.details?.title}</div>
            <div className="hint">{editions.length} édition(s) — {editions.reduce((n, e) => n + (e.volumes?.length || 0), 0)} volume(s)</div>
          </div>

          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8, color: "var(--t1)" }}>➕ Ajouter une édition</div>
            <div style={{ display: "flex", gap: 8 }}>
              <div className="fld" style={{ flex: 1, margin: 0 }}><input value={nef.nom} onChange={e => setNef(f => ({ ...f, nom: e.target.value }))} placeholder="Nom (ex. Édition Deluxe)" /></div>
              <div className="fld" style={{ flex: 1, margin: 0 }}><input value={nef.statut} onChange={e => setNef(f => ({ ...f, statut: e.target.value }))} placeholder="Statut (optionnel)" /></div>
              <button className="btn btn-p" onClick={ajouterEdition}>Ajouter</button>
            </div>
            <div className="card-err">{nefErr}</div>
          </div>

          <div>
            <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8, color: "var(--t1)" }}>📖 Ajouter un volume</div>
            <div className="fld">
              <label>Édition</label>
              <select value={nvEditionId} onChange={e => setNvEditionId(e.target.value)}>
                <option value="">— Choisir —</option>
                {editions.map(e => <option key={e.id} value={e.id}>{e.name || e.nom} ({e.volumes?.length || 0} vol.)</option>)}
              </select>
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <div className="fld" style={{ width: 90 }}><label>N°</label><input value={nvf.numero} onChange={e => setNvf(f => ({ ...f, numero: e.target.value }))} /></div>
              <div className="fld" style={{ flex: 1, minWidth: 140 }}><label>Titre</label><input value={nvf.titre} onChange={e => setNvf(f => ({ ...f, titre: e.target.value }))} /></div>
              <div className="fld" style={{ flex: 1, minWidth: 140 }}><label>Synopsis</label><input value={nvf.synopsis} onChange={e => setNvf(f => ({ ...f, synopsis: e.target.value }))} /></div>
              <div className="fld"><label>Image</label><input type="file" accept="image/*" onChange={e => setNvImage(e.target.files[0] || null)} /></div>
            </div>
            <div className="card-err">{nvErr}</div>
            <button className="btn btn-p" onClick={ajouterVolume}>Ajouter le volume</button>
          </div>
        </>}
      </div>}
    </>
