...
Future<(int, int)> pushNautVolumesToKavita(
  ...
  for (final (nv, effNum) in resolved) {
    ...
    if (title.isEmpty && synopsis.isEmpty && isbn.isEmpty && releaseDate == null &&
        genres.isEmpty && themes.isEmpty && writers.isEmpty && translators.isEmpty && publishers.isEmpty) {
      continue;
    }
    for (final it in matches) {
      ...
      try {
        ...
        if (isbn.isNotEmpty) { updated['isbn'] = isbn; updated['isbnLocked'] = true; }
        if (releaseDate != null) { updated['releaseDate'] = releaseDate; updated['releaseDateLocked'] = true; }
        if (genres.isNotEmpty) { updated['genres'] = genres.map((g) => {'title': g}).toList(); updated['genresLocked'] = true; }
        if (themes.isNotEmpty) { updated['tags'] = themes.map((t) => {'title': t}).toList(); updated['tagsLocked'] = true; }
        if (writers.isNotEmpty) { updated['writers'] = writers.map((n) => {'name': n}).toList(); updated['writerLocked'] = true; }
        if (translators.isNotEmpty) { updated['translators'] = translators.map((n) => {'name': n}).toList(); updated['translatorLocked'] = true; }
        if (publishers.isNotEmpty) { updated['publishers'] = publishers.map((n) => {'name': n}).toList(); updated['publisherLocked'] = true; }
...
class _KavitaScreenState extends State<KavitaScreen> {
  ...
  // Envoie les infos Nautiljon de TOUTES les séries associées vers Kavita
  // en une fois (voir pushNautiljonToKavita) -- une fiche à la fois, avec
  // progression : chaque envoi refait d'abord un GET de la fiche Kavita
  // existante (l'API n'est pas un patch partiel), donc potentiellement long
  ...
  Future<void> _pushAllToServer() async {
    ...
    for (var i = 0; i < matched.length; i++) {
      ...
      if (details != null) {
        final error = await pushNautiljonToKavita(context, seriesId: sid, details: details);
  ...
  // systématiquement vers Kavita) : si une série est à jour, on ne la retouche pas.
  Future<void> _cacheAllVolumes() async {
    ...
    for (var i = 0; i < matched.length; i++) {
      final sid = matched[i]['id'] as int;
      final seriesName = (matched[i]['name'] as String? ?? '');
      if (_naut.cachedVolumes('kavita', '$sid') != null) {
        skipped++;
      } else {
        final match = _naut.matchFor('kavita', '$sid')!;
        try {
...
class _KavitaSeriesDetailScreenState extends State<KavitaSeriesDetailScreen> {
  ...
  void _loadCachedVolumes() {
    ...
    final cached = naut.cachedVolumes('kavita', '${widget.seriesId}');
  ...
  // (voir pickEdition -- si le nom de la série source porte un suffixe
  // d'édition on essaie de le faire correspondre, sinon édition standard).
  // Chargement à la demande (bouton) plutôt qu'automatique à l'ouverture de
  // l'écran : appel réseau de plus, pas systématiquement utile.
  ...
  Future<void> _pushVolumesToServer() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Titre, résumé, ISBN, date de parution, genres, thèmes, auteur, traducteur et éditeur de chaque tome Nautiljon vont être écrits sur le chapitre Kavita du même numéro de volume, puis verrouillés.',
          style: TextStyle(color: AppTheme.t2, fontSize: 12),
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Annuler')),
